...
#---------------------------------start of functions------------------------#
    ...
    def open_file(self):
        ...
        if path != '':
            ...
            with open(path) as csvfile:
                ...
                for rows in file:
                    if rows['town'] not in datasheet:
                        datasheet[rows['town']] = []
                        datasheet[rows['town']].append(rows)
                        headerlist = list(rows.keys())
                        count_entries += 1
                    else:
                        datasheet[rows['town']].append(rows)
                        count_entries += 1 
            ...
            self.tableWidget.setRowCount(count_entries)
            row_count = 0
            column_count = 0
            for towns, listings in datasheet.items():
                for row in listings:
                    for key, value in row.items():
                        self.tableWidget.setItem(row_count, column_count, QTableWidgetItem(value))
                        column_count += 1
                row_count += 1
    ...
    def filter(self):
        # start_time = time.monotonic()
        count = 1
        condition_towns = self.choose_area_combobox.currentText()
        condition_2 = self.house_type_combobox.currentText()
        ...
        if condition_towns == '':
            ...
            self.clear_all()
            filtered_town = datasheet[condition_towns]

           
            
            if condition_3 == 0:
                row_count = 1
                for rows in filtered_town:
                    column_count = 0
                    house_type = rows['flat_type']
                    if house_type == condition_2:
                        self.tableWidget.setRowCount(row_count)
                        for key, values in rows.items():
                            self.tableWidget.setItem(row_count -1, column_count, QTableWidgetItem(str(values)))
                            column_count += 1
                        row_count += 1
            else:
                row_count = 1
                for rows in filtered_town:
                    column_count = 0
                    house_type = rows['flat_type']
                    cost = float(rows['resale_price'])
                    if house_type == condition_2 and cost <= condition_3:
                        self.tableWidget.setRowCount(row_count)
                        for key, values in rows.items():
                            self.tableWidget.setItem(row_count -1, column_count, QTableWidgetItem(str(values)))
                            column_count += 1
